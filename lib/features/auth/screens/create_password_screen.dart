...
/// New password + confirm, with a live strength checklist.
class CreatePasswordScreen extends StatefulWidget {
  const CreatePasswordScreen({required this.activationToken, super.key});
...
class _CreatePasswordScreenState extends State<CreatePasswordScreen> {
  ...
  void _onContinue(BuildContext context) {
    ...
    context.read<ActivateAccountBloc>().add(
      ActivateAccountSubmitted(
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<ActivateAccountBloc, ActivateAccountState>(
      listener: (BuildContext context, ActivateAccountState state) {
      ...
      builder: (BuildContext context, ActivateAccountState state) {
        final bool isLoading = state is ActivateAccountLoading;

        return AuthScaffold(
          showBackground: true,
          dimAmount: 0.55,
          onBack: () => context.go(AppRoutes.checkYourEmail),
